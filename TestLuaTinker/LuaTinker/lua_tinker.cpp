...
template<>
lua_tinker::lua_lstring lua_tinker::read(lua_State *L, int index)
{
	size_t uSize = 0;
	const char* pClstring = lua_tolstring(L,index, &uSize);
	char* plstring = const_cast<char*>(pClstring);
	return lua_lstring(plstring,uSize);
}
